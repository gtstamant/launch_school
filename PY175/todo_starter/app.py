...
from todos.utils import (complete_all_todos,
...
def create_todo(lst, list_id):
    ...
    lst["todos"].append({"title": todo_title,
                         "id": str(uuid4()),
                         "completed": False,
    })

    flash("New todo created!", "success")
    session.modified = True
...
@require_todo
def delete_todo(lst, list_id, todo, todo_id):
...
def mark_all_todos_completed(lst, list_id):
    ...
    flash("All todos complete", "success")
    session.modified = True
    return redirect(url_for("show_list", list_id=list_id))
